...
canvas.addEventListener('mouseup', e=>{
  if(draggingPiece){
    ...
    for(let c of cells){
      ...
      if(d<minDist){minDist=d; closestCell=c;}
    }
    if(closestCell){ draggingPiece.r=closestCell.r; draggingPiece.s=closestCell.s; }
    delete draggingPiece.tempX; delete draggingPiece.tempY;
    draggingPiece=null;
    drawBoard();
...
endTurnBtn.addEventListener('click', ()=>{
  ...
  updateTurnDisplay();
  diceResultDisplay.textContent=''; // サイコロリセット
});
